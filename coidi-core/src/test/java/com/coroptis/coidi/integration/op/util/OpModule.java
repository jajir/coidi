...
import com.coroptis.coidi.op.iocsupport.AuthProcCheckIdImmediate11;
import com.coroptis.coidi.op.iocsupport.AuthProcCheckIdImmediate20;
import com.coroptis.coidi.op.iocsupport.AuthProcCheckIdSetup11;
...
import com.coroptis.coidi.op.iocsupport.OpenIdDispatcher11;
import com.coroptis.coidi.op.iocsupport.OpenIdDispatcher20;
...
public class OpModule extends AbstractModule {
	...
	protected void configure() {
		...
		bind(AuthProc.class).annotatedWith(OpenId11CheckIdSetup.class).to(AuthProcCheckIdSetup11.class);
		bind(AuthProc.class).annotatedWith(OpenId11CheckIdImmediate.class).to(AuthProcCheckIdImmediate11.class);
		bind(AuthProc.class).annotatedWith(OpenId20CheckIdSetup.class).to(AuthProcCheckIdSetup20.class);
		bind(AuthProc.class).annotatedWith(OpenId20CheckIdImmediate.class).to(AuthProcCheckIdImmediate20.class);
		
		
		
		bind(AuthProc.class).annotatedWith(Names.named(AuthProc.AUTH_PROC_SREG_10)).to(AuthProcSreg10.class);
		bind(AuthProc.class).annotatedWith(Names.named(AuthProc.AUTH_PROC_SREG_11)).to(AuthProcSreg11.class);
		bind(AuthProc.class).annotatedWith(Names.named(AuthProc.AUTH_PROC_SIGN)).to(AuthProcSign.class);
		bind(AuthProc.class).annotatedWith(Names.named(AuthProc.AUTH_PROC_VERIFY_LOGGED_USER_11_IMMEDIATE)).to(AuthProcVerifyLoggedUser11Immediate.class);
		bind(AuthProc.class).annotatedWith(Names.named(AuthProc.AUTH_PROC_VERIFY_LOGGED_USER_11_SETUP)).to(AuthProcVerifyLoggedUser11Setup.class);
		bind(AuthProc.class).annotatedWith(Names.named(AuthProc.AUTH_PROC_VERIFY_LOGGED_USER_20_IMMEDIATE)).to(AuthProcVerifyLoggedUser20Immediate.class);
		bind(AuthProc.class).annotatedWith(Names.named(AuthProc.AUTH_PROC_VERIFY_LOGGED_USER_20_SETUP)).to(AuthProcVerifyLoggedUser20Setup.class);
		bind(AuthProc.class).annotatedWith(Names.named(AuthProc.AUTH_PROC_VERIFY_IDENTITY_11)).to(AuthProcVerifyLoggedUser11Immediate.class);
		bind(AuthProc.class).annotatedWith(Names.named(AuthProc.AUTH_PROC_VERIFY_IDENTITY_20)).to(AuthProcVerifyIdentity20.class);
		bind(AuthProc.class).annotatedWith(Names.named(AuthProc.AUTH_PROC_ASSOCIATION)).to(AuthProcAssociation.class);
		bind(AuthProc.class).annotatedWith(Names.named(AuthProc.AUTH_PROC_STATE_LESS_ASSOCIATION)).to(AuthProcStateLessAssociation.class);
		bind(AuthProc.class).annotatedWith(Names.named(AuthProc.AUTH_PROC_NONCE)).to(AuthProcNonce.class);
		bind(AuthProc.class).annotatedWith(Names.named(AuthProc.AUTH_PROC_IDENTITY_11)).to(AuthProcIdentity11.class);
		bind(AuthProc.class).annotatedWith(Names.named(AuthProc.AUTH_PROC_IDENTITY_20)).to(AuthProcIdentity20.class);
