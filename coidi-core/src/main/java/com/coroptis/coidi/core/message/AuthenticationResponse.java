...
public class AuthenticationResponse extends AbstractOpenIdRequest {
	...
	public String toString() {
		return MoreObjects.toStringHelper(AuthenticationResponse.class).add(IDENTITY, getIdentity())
				.add(CLAIMED_ID, getClaimedId()).add(NONCE, getNonce()).add(RETURN_TO, getReturnTo())
				.add(SIGNED, getSigned()).add(SIG, getSignature()).toString();
	}

	@Override
	public String getUrl(final String targetUrl) {
		if (targetUrl == null) {
			return getUrlMessage(OPENID, getReturnTo());
		} else {
