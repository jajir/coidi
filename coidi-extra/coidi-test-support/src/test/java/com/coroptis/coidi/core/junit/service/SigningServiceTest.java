...
public class SigningServiceTest extends BaseJunitTest {
	...
	public void testSign() throws Exception {
		...
		EasyMock.expect(services.getMessageService().extractStringForSign(response, null)).andReturn("toSign");
		EasyMock.expect(services.getConvertorService().convertToBytes("3uAPFsOq3UYQ3v3SeujCZaNclIg=")).andReturn(bytes);
		EasyMock.expect(services.getCryptographyService().generateMac(EasyMock.eq(bytes), (byte[]) EasyMock.anyObject(),
				EasyMock.eq(AssociationType.HMAC_SHA1))).andReturn(signature);
		EasyMock.expect(services.getConvertorService().convertToString(signature)).andReturn("signature");
		services.replay();
