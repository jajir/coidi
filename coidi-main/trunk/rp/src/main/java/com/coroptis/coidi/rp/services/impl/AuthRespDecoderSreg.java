...
public class AuthRespDecoderSreg implements AuthRespDecoder, NsSreg {
	...
	@Override
	public Boolean decode(AuthenticationResponse authenticationResponse,
			Association association, AuthenticationResult authenticationResult) {
		...
		if (authenticationResponse.get(SREG_DOB) != null) {
			...
			try {
				resultSreg.setDob(sdf.parse(authenticationResponse
						.get(SREG_DOB)));
		...
		if (touche) {
			authenticationResult.getExtensions().put(ExtensionResultSreg.CODE,
					resultSreg);
		}
		return true;
	}
